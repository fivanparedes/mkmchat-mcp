...
class ExplainMechanic extends Component
{
    public string $mechanic = '';
    public string $modelSlug = '';
    public bool $loading = false;
    public ?string $errorMessage = null;
    /** @var array{definition: string, recommendations: string}|null */
    ...
    {
        Gate::authorize('explain-mechanic');

        $this->validate([
            'mechanic'  => ['required', 'string', 'min:3', 'max:2000'],
        ...
        $history = QueryHistory::create([
            'user_id'    => $user->id,
            'query_type' => 'explain_mechanic',
            'strategy'   => $this->mechanic,
            'model_slug' => $this->modelSlug,
            'status'     => 'pending',
        ]);

        try {
            $payload = $apiService->explainMechanic($this->mechanic, $this->modelSlug);
            $history->update(['response' => $payload, 'status' => 'success']);
            $this->result    = $payload;
    ...
    {
        return view('livewire.explain-mechanic');
    }

    private function rateLimitKey(User $user): string
    {
        $ip = request()->ip() ?? 'unknown';
        return "explain-mechanic:{$user->id}:{$ip}";
    }
}
